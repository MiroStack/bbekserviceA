...
public interface UserProfileRepo extends JpaRepository<UserProfileEntity, Long> {
    ...
    @Query(value = "SELECT\n" +
            ...
            "  COUNT(*) OVER() AS total_rows\n" +
            "FROM bbek.user_profile AS up\n" +
            "LEFT JOIN rf_position AS rp ON up.position_id = rp.id\n" +
            "LEFT JOIN rf_department AS rd ON up.department_id = rd.id\n" +
            "LEFT JOIN baptism AS ba ON up.id = ba.profile_id\n" +
            "LEFT JOIN tbl_member AS tm ON up.id = tm.profile_id\n" +
            "WHERE up.role_id = 1\n" +
            "  AND (\n" +
    ...
    List<ModifiedUserProfile> findDepartmentsMembers(@Param("query") String query, @Param("numberOfRowsToSkip") int numberOfRowsToSkip);
}
